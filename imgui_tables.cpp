...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    if (width_remaining_for_stretched_columns >= 1.0f && !(table->Flags & ImGuiTableFlags_PreciseWidths))
        for ( order_n = table->ColumnsCount - 1; stretch_sum_weights > 0.0f && width_remaining_for_stretched_columns >= 1.0f && order_n >= 0; order_n--)
        {
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for ( n = 0; n < IM_ARRAYSIZE(merge_groups); n++)
    ...
    for ( column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        for ( merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
...
#if 0
    if (g.IO.KeyShift)
        for ( merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            MergeGroup* merge_group = &merge_groups[merge_group_n];
            if (merge_group->ChannelsCount == 0)
                continue;
    ...
    if (merge_group_mask != 0)
    {
        ...
        for ( merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                for ( n = 0; n < (size_for_masks_bitarrays_one >> 2); n++)
        ...
        for ( n = 0; n < splitter->_Count && remaining_count != 0; n++)
        {
...
static void TableSettingsInit(ImGuiTableSettings* settings, ImGuiID id, int columns_count, int columns_count_max)
{
	...
	memset(settings, 0, sizeof(ImGuiTableSettings)); // メモリをゼロクリア
	settings->ImGuiTableSettings::ImGuiTableSettings(); // コンストラクタを呼び出す
    ImGuiTableColumnSettings* settings_column = settings->GetColumnSettings();
//    for ( n = 0; n < columns_count_max; n++, settings_column++)
//        IM_PLACEMENT_NEW(settings_column) ImGuiTableColumnSettings();
	for (n = 0; n < columns_count_max; n++, settings_column++)
	{
		memset(settings_column, 0, sizeof(ImGuiTableColumnSettings)); // メモリをゼロクリア
		settings_column->ImGuiTableColumnSettings::ImGuiTableColumnSettings(); // コンストラクタを呼び出す
	}
    settings->ID = id;
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    for ( n = 0; n < table->InstanceCurrent + 1; n++)
    {
