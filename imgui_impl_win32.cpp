...
#endif
#ifndef _WIN32_WINNT
#define _WIN32_WINNT 0x0500 // Windows 2000以降の機能を有効にする
#endif
#include <windows.h>
...
#define nullptr NULL 
#ifndef IDC_HAND
#define IDC_HAND MAKEINTRESOURCE(32649) // Windows 2000以降のIDC_HANDの値
#ifndef _WIN32_WINNT
#define _WIN32_WINNT 0x0500 // Windows 2000以降の機能を有効にする
#endif
...
#if defined(_MSC_VER) && (_MSC_VER >= 1400) // VC++ 2005以降
#include <dwmapi.h>
#endif
...
//    ImGui_ImplWin32_Data* bd = IM_NEW(ImGui_ImplWin32_Data)();
	ImGui_ImplWin32_Data* bd = (ImGui_ImplWin32_Data*)ImGui::MemAlloc(sizeof(ImGui_ImplWin32_Data));
	if (bd != NULL)
	{
		bd->ImGui_ImplWin32_Data::ImGui_ImplWin32_Data(); // コンストラクタを呼び出す
	}
...
typedef BOOL (CALLBACK* MONITORENUMPROC)(HMONITOR, HDC, LPRECT, LPARAM);
    ...
    // GetMonitorInfoW関数のプロトタイプを追加
    typedef BOOL(WINAPI* LPFN_GETMONITORINFOW)(HMONITOR, LPMONITORINFO);

    // GetMonitorInfoW関数を動的にロードする関数
    BOOL MyGetMonitorInfoW(HMONITOR hMonitor, LPMONITORINFO lpmi)
    {
        static HMODULE hUser32 = LoadLibrary(TEXT("user32.dll"));
        static LPFN_GETMONITORINFOW pGetMonitorInfoW = (LPFN_GETMONITORINFOW)GetProcAddress(hUser32, "GetMonitorInfoW");
        if (pGetMonitorInfoW)
            return pGetMonitorInfoW(hMonitor, lpmi);
        return FALSE;
    }
    // EnumDisplayMonitors関数のプロトタイプを追加
    typedef BOOL(WINAPI* LPFN_ENUMDISPLAYMONITORS)(HDC, LPCRECT, MONITORENUMPROC, LPARAM);

    // EnumDisplayMonitors関数を動的にロードする関数
    BOOL MyEnumDisplayMonitors(HDC hdc, LPCRECT lprcClip, MONITORENUMPROC lpfnEnum, LPARAM dwData)
    {
        static HMODULE hUser32 = LoadLibrary(TEXT("user32.dll"));
        static LPFN_ENUMDISPLAYMONITORS pEnumDisplayMonitors = (LPFN_ENUMDISPLAYMONITORS)GetProcAddress(hUser32, "EnumDisplayMonitors");
        if (pEnumDisplayMonitors)
            return pEnumDisplayMonitors(hdc, lprcClip, lpfnEnum, dwData);
        return FALSE;
    }
...
#if _MSC_VER <= 1200 
    if (!MyGetMonitorInfoW(monitor, &info))
#else
...
#if _MSC_VER <= 1200 
    MyEnumDisplayMonitors(nullptr, nullptr, ImGui_ImplWin32_UpdateMonitors_EnumFunc, 0);
#else
...
typedef HMONITOR (WINAPI *LPFN_MONITORFROMWINDOW)(HWND, DWORD);
#ifndef MONITOR_DEFAULTTONEAREST
#define MONITOR_DEFAULTTONEAREST 0x00000002
#endif
HMONITOR MyMonitorFromWindow(HWND hwnd, DWORD dwFlags)
{
    static HMODULE hUser32 = LoadLibrary(TEXT("user32.dll"));
    static LPFN_MONITORFROMWINDOW pMonitorFromWindow = (LPFN_MONITORFROMWINDOW)GetProcAddress(hUser32, "MonitorFromWindow");
...
{
#if _MSC_VER <= 1200 
    HMONITOR monitor = MyMonitorFromWindow((HWND)hwnd, MONITOR_DEFAULTTONEAREST);
#else
...
#endif
// DwmIsCompositionEnabled関数のプロトタイプを追加
typedef HRESULT (WINAPI *LPFN_DWMISCOMPOSITIONENABLED)(BOOL*);

BOOL MyDwmIsCompositionEnabled(BOOL* pfEnabled)
{
    static HMODULE hDwmapi = LoadLibrary(TEXT("dwmapi.dll"));
    static LPFN_DWMISCOMPOSITIONENABLED pDwmIsCompositionEnabled = (LPFN_DWMISCOMPOSITIONENABLED)GetProcAddress(hDwmapi, "DwmIsCompositionEnabled");
    if (pDwmIsCompositionEnabled)
        return pDwmIsCompositionEnabled(pfEnabled);
    return FALSE;
}
// DwmGetColorizationColor関数のプロトタイプを追加
typedef HRESULT (WINAPI *LPFN_DWMGETCOLORIZATIONCOLOR)(DWORD*, BOOL*);

HRESULT MyDwmGetColorizationColor(DWORD* pcrColorization, BOOL* pfOpaqueBlend)
{
    static HMODULE hDwmapi = LoadLibrary(TEXT("dwmapi.dll"));
    static LPFN_DWMGETCOLORIZATIONCOLOR pDwmGetColorizationColor = (LPFN_DWMGETCOLORIZATIONCOLOR)GetProcAddress(hDwmapi, "DwmGetColorizationColor");
    if (pDwmGetColorizationColor)
        return pDwmGetColorizationColor(pcrColorization, pfOpaqueBlend);
    return E_FAIL;
}
// DwmEnableBlurBehindWindow関数のプロトタイプを追加
typedef HRESULT (WINAPI *LPFN_DWMENABLEBLURBEHINDWINDOW)(HWND, const DWM_BLURBEHIND*);

HRESULT MyDwmEnableBlurBehindWindow(HWND hWnd, const DWM_BLURBEHIND* pBlurBehind)
{
    static HMODULE hDwmapi = LoadLibrary(TEXT("dwmapi.dll"));
    static LPFN_DWMENABLEBLURBEHINDWINDOW pDwmEnableBlurBehindWindow = (LPFN_DWMENABLEBLURBEHINDWINDOW)GetProcAddress(hDwmapi, "DwmEnableBlurBehindWindow");
...
void ImGui_ImplWin32_EnableAlphaCompositing(void* hwnd)
{
    ...
    BOOL composition;
#if _MSC_VER <= 1200 
    if (FAILED(MyDwmIsCompositionEnabled(&composition)) || !composition)
#else
    ...
    DWORD color;
#if _MSC_VER <= 1200 
    if (_IsWindows8OrGreater() || (SUCCEEDED(MyDwmGetColorizationColor(&color, &opaque)) && !opaque))
#else
...
#endif
    {
        ...
        bb.fEnable = TRUE;
#if _MSC_VER <= 1200 
        MyDwmEnableBlurBehindWindow((HWND)hwnd, &bb);
#else
...
//    ImGui_ImplWin32_ViewportData* vd = IM_NEW(ImGui_ImplWin32_ViewportData)();
	ImGui_ImplWin32_ViewportData* vd = (ImGui_ImplWin32_ViewportData*)ImGui::MemAlloc(sizeof(ImGui_ImplWin32_ViewportData));
	if (vd != NULL)
	{
		vd->ImGui_ImplWin32_ViewportData::ImGui_ImplWin32_ViewportData(); // コンストラクタを呼び出す
	}
...
#define GWLP_HWNDPARENT (-8) // 定数を手動で定義
#endif
#ifndef LONG_PTR
#ifdef _WIN64
...
// SetLayeredWindowAttributes関数のプロトタイプを追加
typedef BOOL (WINAPI *LPFN_SETLAYEREDWINDOWATTRIBUTES)(HWND, COLORREF, BYTE, DWORD);

BOOL MySetLayeredWindowAttributes(HWND hwnd, COLORREF crKey, BYTE bAlpha, DWORD dwFlags)
{
    static HMODULE hUser32 = LoadLibrary(TEXT("user32.dll"));
    static LPFN_SETLAYEREDWINDOWATTRIBUTES pSetLayeredWindowAttributes = (LPFN_SETLAYEREDWINDOWATTRIBUTES)GetProcAddress(hUser32, "SetLayeredWindowAttributes");
...
#if _MSC_VER <= 1200 
        MySetLayeredWindowAttributes(vd->Hwnd, 0, (BYTE)(255 * alpha), LWA_ALPHA);
#else
