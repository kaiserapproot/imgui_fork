...
#if defined(_MSC_VER) && !defined(__clang__)  && !defined(__INTEL_COMPILER) && !defined(IMGUI_DEBUG_PARANOID) && _MSC_VER != 1200
...
struct ImVec2
{
    ...
    bool operator<(const ImVec2& rhs) const { return x < rhs.x && y < rhs.y; }
    bool operator>(const ImVec2& rhs) const { return x > rhs.x && y > rhs.y; }
    bool operator<=(const ImVec2& rhs) const { return x <= rhs.x && y <= rhs.y; }
    bool operator>=(const ImVec2& rhs) const { return x >= rhs.x && y >= rhs.y; }
...
struct ImVec4
{
    ...
    ImVec4 operator*(float rhs) const { return ImVec4(x * rhs, y * rhs, z * rhs, w * rhs); }
